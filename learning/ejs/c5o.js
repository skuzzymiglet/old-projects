...
//let shitAppender = appender(", shit!");
//console.log(shitAppender("my mum"));
//console.log(minArgs(repeat, [5, console.log]));
callAndDebug((arg1, arg2) => {return arg1 === arg2}, ["yeet", 56])
...
newScripts.map(x => ({century: Math.floor(x.year/100), name: x.name}));

console.log(newScripts[0].century, newScripts[newScripts.length - 1].century);
for(let i = newScripts[0].century; i <= newScripts[newScripts.length - 1].century; i++){
    ...
    if(currentScripts.length == 0){
        ...
        currentScripts.forEach(x => console.log(x.name));
    }
}
